...
$sql="select * from questions where type=1 and gender='$_SESSION[gender]' limit 10";
...
$(".Qselect").change(function()
{
  ...
  $currentq=$(this).siblings('.answermaindiv');
  $.ajax({

          type: "POST",
          url: "Services/extraQoptionchangeService.php",
          ...
          debugger;
          $currentq.html('');
          $option=jQuery.parseJSON(e);
          for($i=0;$i<$option.length;$i++)
          {
               $id=$option[$i].replace(/\s/g, '');
               $string='<div class="row mt-2" ><div class="answerdiv  "><input type="radio" class="option-input mb-3 radio radioanswer_'+$q_id+'" name="radioanswer_'+$q_id+'" >';
               $string+='&nbsp;<textarea row="1" name="answer_'+$q_id+'_'+$i+'" class="font-22 answerwidth  p-2 textq " placeholder="Write Your answer here" autofocus>'+$option[$i]+'</textarea></div>';
               $string+='<div class=" crossbutton"><button type="button" class="btn delbutton" style="color:white" name="button">X</button></div>';
               $currentq.append($string);
            }
